...
module Theory.SC+El+Pi+Id.QIIRT.Elimination where
...
module _ {ℓ ℓ′ : Level} (elim : Eliminator {ℓ} {ℓ′}) where
  ...
  elimTy[] {Δ} {Γ} σ (Π A B) =
    ...
    where
      open ≡-Reasoning
      eq : _≡_ {A = ∃ λ PB → Tyᴹ (elimCtx Δ ,ᶜᴹ PB) ([ σ ↑ A ] B)}
  ...
  elimSub↑-tot idS A = begin
    ...
    elimTy ([ idS ] A) ,' elimSub (idS ↑ A)
      ∎
    where open ≡-Reasoning
  elimSub↑-tot {Δ} {Γ} (_⨟_ {Δ = Θ} σ τ) A = begin
  ...
  elimSub↑-tot {Δ} {Γ} (π₁ {A = A'} idS) A = ≅-to-≡ $ begin
