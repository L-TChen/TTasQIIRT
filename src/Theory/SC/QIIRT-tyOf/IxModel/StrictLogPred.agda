...
ι' : {Γᴾ : Ctxᴾ Γ} → {A : Ty Γ} → Tyᴾ Γᴾ A → Ty (ctxᴾ Γᴾ ,C A [ wkᴾ Γᴾ ]T)
ι' (Δ , r , Aᴾ) = Aᴾ [ subst (Sub _) (sym r) idS ]T
...
ι'ι⁻¹ : {Γᴾ : Ctxᴾ Γ} → {A : Ty Γ} → (A : Ty (ctxᴾ Γᴾ ,C A [ wkᴾ Γᴾ ]T)) → ι' (ι⁻¹ A) ≡ A
ι'ι⁻¹ A = cong (A [_]T) (substRefl {B = (Sub _)} idS)
...
≡-Tyᴾ : {Γᴾ : Ctxᴾ Γ} → {A : Ty Γ} →  (Aᵖ Bᴾ : Tyᴾ Γᴾ A) → ι Aᵖ ≡ ι Bᴾ → Aᵖ ≡ Bᴾ
≡-Tyᴾ Aᵖ Bᵖ p = sym (ι⁻¹ι Aᵖ) ∙ cong ι⁻¹ p ∙ ι⁻¹ι Bᵖ
...
LogPredᵐ .tyOf[]∙ {Δ∙ = Δ∙} {t} {Γ∙ = Γ∙} {σ} {t∙ = t∙@(Aᵖ , tᵖ , eᵖ)} {σ∙ = σ∙} = ≡-Tyᴾ _ _ (ιι⁻¹ _ ∙ tyOfπ₂ idS ∙ goal)
 where
  goal : tyOf t [ σ ∘ wkᴾ Γ∙ ∘ π₁ idS ]T ≡ subst Ty (λ i → Γ∙ .ctxᴾ ,C [wkᴾnat] (tyOf t) σ∙ i) (ι (tyOfᴾ t∙) [ subᴾ σ∙ ↑ (tyOf t [ wkᴾ Δ∙ ]T) ]T)
  goal = cong (λ z → tyOf t [ z ∘ π₁ idS ]T) (sym (wkᴾnat σ∙)) ∙ {!snd t∙!}
   where
    goal' : tyOf t [ wkᴾ Δ∙ ∘ subᴾ σ∙ ∘ π₁ idS ]T ≡ subst Ty (λ i → Γ∙ .ctxᴾ ,C [wkᴾnat] (tyOf t) σ∙ i) (ι Aᵖ [ subᴾ σ∙ ↑ (tyOf t [ wkᴾ Δ∙ ]T) ]T)
    goal' = {!!}
...
LogPredᵐ .[∘]t∙ t∙ σ∙ τ∙ = {!!}
LogPredᵐ .U∙ = ι⁻¹ U
LogPredᵐ .U[]∙ {Γ} {Δ} {σ} {Γ∙} {Δ∙} {σ∙} = ≡-Tyᴾ _ _ {!!}
 where
  lemma : subst Ty (λ i → Γ∙ .ctxᴾ ,C [wkᴾnat] U σ∙ i) (ι (ι⁻¹ U) [ subᴾ σ∙ ↑ (U [ wkᴾ Δ∙ ]T) ]T) ≡ ι (ι⁻¹ U)
  lemma = {!!}
  claim : (x : Ty (Γ∙ .ctxᴾ ,C U [ wkᴾ Δ∙ ]T [ subᴾ σ∙ ]T))
        → subst Ty (λ i → Γ∙ .ctxᴾ ,C [wkᴾnat] U σ∙ i) x ≡ x [ π₁ idS , π₂ idS ∶[ tyOfπ₂ idS ∙ {!!} ] ]T
  claim = {!!}
