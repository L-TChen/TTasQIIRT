...
module Theory.SC.QIIRT-*of.Syntax where
...
--      : Tm → Ctx
  ...
  data Ty where
    _[_]
      : (A : Ty Δ)(σ : Sub Γ) ⦃ p : cod σ ≡ Δ ⦄
      → Ty Γ
  data Sub where
    ∅S
      : Sub Γ
    _,_∶_
      : (σ : Sub Γ) ⦃ p : cod σ ≡ Δ ⦄ (t : Tm Γ)(A : Ty Δ)
      → ⦃ q : tyOf t ≡ A [ σ ] ⦄
      → Sub Γ
    idS
      : Sub Γ
    _∘_
      : Sub Δ → (τ : Sub Γ) ⦃ pτ : cod τ ≡ Δ ⦄
      → Sub Γ
    π₁
      : (σ : Sub Γ) (Δ : Ctx) (A : Ty Δ)
      → ⦃ cod σ ≡ (Δ , A) ⦄
      → Sub Γ
  ...
  cod (σ ∘ τ)     = cod σ
  cod (π₁ σ Δ A)  = Δ
  tyOf (π₂ σ Δ A) = _[_] A (π₁ σ Δ A) ⦃ refl ⦄
  tyOf (t [ σ ]t) = tyOf t [ σ ]
  postulate
    assoc
      : (σ : Sub Γ) ⦃ pσ : cod σ ≡ Δ ⦄ (τ : Sub Δ) ⦃ pτ : cod τ ≡ Θ ⦄ (γ : Sub Θ) 
      → (γ ∘ τ) ∘ σ ≡ γ ∘ (τ ∘ σ)
--     tyOfπ₂
