...
module Foo where
  module _ where -- delimit the scope of forward declarations
    ...
    ,∘
      ...
      → (σ ,S t) ∘ τ
      ≡ (σ ∘ τ ,S subst (Tm Γ) ([∘]T A τ σ) (t [ τ ]t))
    ηπ
      : (σ : Sub Γ (Δ , A))
      → σ ≡ (π₁ σ ,S π₂ σ)
    η∅
    ...
    _↑_
      ...
      → Sub (Γ , A [ σ ]T) (Δ , A)
    _↑_ {Γ} σ A =
      σ ∘ π₁ idS ,S subst (Tm (Γ , (A [ σ ]T))) ([∘]T _ _ _) (π₂ idS)
    βπ₂
      : (σ : Sub Γ Δ) (t : Tm Γ (A [ σ ]T))
      → π₂ (σ ,S t)
...
--      Ty-is-set : isSet (Ty Γ)
    ...
    data Sub where
      ...
      ,∘'
        : (σ : Sub Δ Θ) (t : Tm Δ (A [ σ ]T)) (τ : Sub Γ Δ)
        → (σ ,S t) ∘ τ ≡
          (σ ∘ τ ,S subst (Tm Γ) ([∘]T _ τ σ) (t [ τ ]t))
      η∅'
        : (σ : Sub Γ ∅)
        → σ ≡ ∅S
      ηπ'
...
π∘ {Γ} {Δ} {A} {Ξ} {τ} {σ} = 
 ...
 A [ π₁ ((π₁ τ , π₂ τ) ∘ σ) ] ,, π₂ ((π₁ τ , π₂ τ) ∘ σ)
   ≡⟨ {!!} ⟩
 A [ π₁ ((π₁ τ ∘ σ) , subst (Tm Γ) ([∘]T _ _ _) (π₂ τ [ σ ])) ] ,,
   π₂ ((π₁ τ ∘ σ) , subst (Tm Γ) ([∘]T _ _ _) (π₂ τ [ σ ]))
   ≡⟨ {!!} ⟩
 A [ π₁ τ ∘ σ ] ,, subst (Tm Γ) ([∘]T _ _ _) (π₂ τ [ σ ])
   ≡⟨ {!!} ⟩
 A [ π₁ τ ] [ σ ] ,, π₂ τ [ σ ]
   ∎
