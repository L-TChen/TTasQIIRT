...
module <>≡id where
  ...
  <>s {Γ}  ∅                    = hcong {B = λ Γ → Sub Γ ∅} (λ Γ → ∅ {Γ}) (≡-to-≅ $ <>c Γ)
  <>s {Γ} (_,_ {Δ} {i} σ {A} t) = icong₂ {I = Ctx × ∃ (λ Δ → Ty Δ i)}
                                         (λ (Γ , (Δ , _)) → Sub Γ Δ)
                                         {λ {(Γ , (Δ , A))} σ → Tm Γ ([ σ ] A)}
    ...
    where
      ...
      _,ˢ_ = _,_
  <>s {Γ}  idS                  = hcong (λ Γ → idS {Γ}) (≡-to-≅ $ <>c Γ)
  <>s {Γ} (_⨟_ {Δ} {Θ} τ σ)     = icong₂ {I = Ctx × Ctx × Ctx}
                                         (λ (Γ , (Δ , Θ)) → Sub Γ Δ)
                                         {λ {(Γ , (Δ , Θ))} _ → Sub Δ Θ}
                                         ...
                                         (<>s τ)
                                         (<>s σ)
  <>s {Γ} (π₁ {Δ} {i} {A} σ)    = icong {I = Ctx × ∃ (λ Δ → Ty Δ i)}
                                        (λ (Γ , (Δ , A)) → Sub Γ (Δ , A))
                                        (cong₂ _,_ (<>c Γ) (<>c Δ ≡,≅ <>ty A))
                                         π₁
                                        (<>s σ)  
  <>tm {Γ}         (π₂ {_} {Δ} {i} {A} σ)   = HEq.trans (tr≅ (Tm ((Γ <c) >c)) ([]>ty (IR.π₁ (σ <s)) (A <ty)) (π₂ ((σ <s) >s))) 
                                                        (icong {I = Ctx × ∃ (λ Δ → Ty Δ i)}
                                                               (λ (Γ , (Δ , A)) → Sub Γ (Δ , A))
                                                               (cong₂ _,_ (<>c Γ) (<>c Δ ≡,≅ <>ty A))
                                                                π₂
                                                               (<>s σ))
  <>tm {Γ} {i}     ([_]tm_ {Δ = Δ} σ {A} t) = HEq.trans (≡-to-≅ $ sym $ []>tm (σ <s) (t <tm))
