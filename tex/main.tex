...
We can also define join lists as a quotient inductive-\emph{recursive} type, as follows: the type $\JList{A}$ is defined simultaneously with a recursive function $\flatten : \implicit{A}\;\JList{A} \to \List{A}$ which flattens a join list into an ordinary list. We keep the constructors $\Jnil$, $\Jsing{\blank}$ and $\Jconcat{\blank}{\blank}$ from \cref{fig:QIT-join-list}, but replace $\mathsf{idl}$, $\mathsf{idr}$ and $\mathsf{assoc}$ by a single constructor
\[
...
For example, the two definitions of addition $\blank  + \blank$, $\blank  +' \blank $
%
%\begin{figure}[b]%
...
\subTy{\UU}{\sigma}  & \defeq \UU                                   \label{eq:type-sub-at2-1} \\
\subTy{A}{\idS}    & \defeq A                                     \label{eq:type-sub-at2-2} \\
\subTy{A}{\sigma ; \tau}    & \defeq \subTy{(\subTy{A}{\tau})}{\sigma} \label{eq:type-sub-at2-3}
\end{alignat}
...
\begin{proof}
  ...
  For local confluence, we consider all local peaks, i.e., each combination of \eqref{eq:def-type-subst-5} with \eqref{eq:def-type-subst-1}--\eqref{eq:def-type-subst-4},
  \[
...
Instead, we define the \emph{multiple lifting} $\sigma \upuparrows \Xi$ of $\sigma$ by a telescope $\Xi = A_1, \dots, A_n$, given by multiple applications of lifting $\sigma \uparrow A_1 \uparrow \dots \uparrow A_n$.
...
The type of telescopes $\Tel : \Ctx \to \Set$ over a `base' context $\Gamma$ are defined inductive-recursively together with a concatenation $\blank \dplus\blank : (\Gamma : \Ctx) (\Xi : \Tel\;\Gamma) \to \Ctx$ as follows.
...
\]
which requires us to also simultaneously define telescope substitution $\subTel{\blank}{\blank} :  \Tel\;\Delta \to \Sub\;\Gamma\;\Delta \to \Tel\;\Gamma$.
...
%
Instead of resorting to \transp, we also turn telescope substitution $\subTel{\blank}{\blank}$ and the multiple lifting into definitions by overlapping patterns by adding the following clauses:
...
{proposition}{coherenceTel}
  ...
  $\subTel{\Xi}{\sigma} = \subTel{\Xi}{\tau}$ and $\sigma \upuparrows \Xi =^{\Sub\;(\Gamma\dplus \subTel{\Xi}{\sigma})\;(\Delta\dplus\Xi)} \tau \upuparrows \Xi$.
  \danger
\end{restatable}
...
$t \mathop{\$} u \defeq \subTm{(\mathsf{app}\,t)}{(\idS , t)}$ of type $\Tm\,\Gamma\,(\sub{B}{(\idS , t)})$ can be defined without any transport. This is in contrast to the QIIT situation, where ordinary application is defined as $t \mathop{\$} u \defeq \subTm{(\mathsf{app}\,t)}{(\idS, \alert{\transfib{\Tm\;\Gamma}{[id]_T^{-1}}{\color{black}t}})}$.
%
%\begin{alignat*}{3}
...
%\end{align*}
can be derived without transport (cf.\ Altenkirch and Kaposi~\cite{Altenkirch2016a}), since \eqref{eq:def-type-subst-8} is now strict.
...
\end{alignat*}
%
as constructors, and the structural rule for type substitution as another straightforward clause
...
Note that a simultaneously defined function does not need a corresponding interpretation for Dybjer and Setzer's inductive-recursive types~\cite{Dybjer1999}, as their framework targets meta-theoretic types rather than the inductive types being defined.
\end{remark}
...
\begin{alignat*}{5}
  ...
  [\emptysub]^M_{\Tm} & : t^M [\emptysub^M_{\Sub}]^M_{\Tm}                   && = %^{\Tm^M\,\Gamma^M\;([\emptysub_{\Sub}^M]^M_{\Ty}\,A^M)\,([\emptysub]_{\Tm}\,t)}
                      t^M [ \emptysub^M_{\Sub} ]^M  &&  \\
  [,]_{\Tm}^M         & : t^M [ \sigma^M ,_{\Sub}^M u^M ]^M_{\Tm}            && = %^{\Tm^M\,\Gamma^M\;([\sigma^M ,_{\Sub}^M t ]^M_{\Ty}\,A^M)\,([\sigma , t]_{\Tm}\,t)}
                      t^M [ \sigma^M ,_{\Sub}^M u^M ]^M && \\
  [\pi_1\idS]^M_{\Tm} & : t^M [ \pi_1^M\,\idS^M ]^M_{\Tm}                    && = %^{\Tm^M\,(\Gamma^M ,^M_{\Ctx} A^M)\,([\pi_1^M\,\idS^M ]^M_{\Ty})\,A^M)\;([\pi_1\idS]_{\Tm}\,t)}{}
                      t^M [ \pi_1^M\,\idS^M ]^M && \\
  [\pi_1\pi_1]^M_{\Tm}& : t^M [ \pi_1^M\,(\pi_1^M\,\sigma^M) ]^M_{\Tm}       && = %^{\Tm^M\,\Gamma^M\,([\pi_1^M\,(\pi_1^1\,\sigma^M)]^M_{\Ty}\,A)\;([\pi_1(\pi_1\,\sigma)]_{\Tm}\,t)}
