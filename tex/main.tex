...
\documentclass[a4paper,UKenglish,numberwithinsect,cleveref,thm-restate,final]{lipics-v2021}
...
\begin{abstract}
  ...
  As a case study, we investigate substitution calculus and type theory with a universe and $\Pi$-types, and compare ours with quotient inductive-inductive definitions.
\end{abstract}
...
Our definition has the standard model, justifying its soundness of our definition, given by the same motives as those for its QII counterpart~\cite[Section~4]{Altenkirch2016a}:
\begin{align*}
  \Ctx^M & \defeq \lam{\nameless} \Set &
  \Sub^M & \defeq \lam{\Gamma^M}{\Delta^M}{\nameless} \Gamma^M \to \Delta^M \\
...
\begin{align*}
  \blank\uparrow^M\blank & \judgeq \lam{\sigma^M}{A^M}{\gamma} (\sigma^M\,(\pi_1\,\gamma) , \pi_2\,\gamma) & \\
...
In the QII counterpart, none of identities hold definitionally, because their indices always involve equality constructors.
For example, the identity type substitution corresponds to
\begin{alignat*}{5}
  [\idS]^M & : [ \idS^M ]^M_{\Ty}\, A^M && =^{\Ty^M\;\Gamma^M}_{\alert{[\idS]}} A^M
\end{alignat*}
The transport along $[\idS] : [\idS]\,A = A$ above is needed, because $[ \idS^M ]^M_{\Ty}\, A^M$ is indexed by $[\idS]\,A$ instead of $A$.
Accordingly, this identity becomes $\lam{\gamma} A^M\,\gamma =_{[\idS]}^{\lam{\nameless}\Gamma^M \to \Set} A^M$ in the standard model and requires a lemma about constant transport~\cite[Lemma~2.3.5]{UFP2013} to show.

In contrast, in our QIIR definition, many identities just hold definitionally.
Consider
\begin{alignat*}{5}
...
\end{alignat*}
where $[\idS]^M$ is just $(\lam{\gamma} A^M\,\gamma) = A^M$ for $A^M : \Gamma^M \to \Set$ in the model and holds definitionally by the $\eta$-law for $\Pi$-types.
Hence, $\idS{\mathord{\uparrow}}^M$ becomes $(\idS^M \uparrow^M A^M) = \idS^M$, definitionally equal to $(\lam{\gamma}(\pi_1\gamma, \pi_2\gamma)) = \lam{\gamma}\gamma$, and holds definitionally by the $\eta$-law for $\Sigma$-types.

Additional rules we have used to enable further reduction are derivable already, and they can be justified by the same standard model with simpler proofs.
