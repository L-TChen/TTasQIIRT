...
\end{proposition}
\begin{proof}
  We demonstrate the proof for $\pi_1\beta: \pi_1(\sigma, t) = t$ and other three cases follow similarly.

  First, we prove the coherence for $[\blank]_{\Ty}\,A$ by induction on $A$, assuming the coherence for the other two identities $[\blank]_{\Tel}\, \Xi$ and $\blank \upuparrows \Xi$, which are then proved by induction on $\Xi$.
  \begin{enumerate}
    \item By induction on $A$, consider the following cases for $[\sigma\upuparrows\Xi]_{\Ty}\,A = [\tau \upuparrows \xi]_{\Ty}\,A$.
      \begin{description}
        \item[$A$ is $\UU$:] By definition
          ...
          \]
          By assumption, $[\pi_1(\sigma, t)]_{\Tel}\,\Xi = [ \sigma]_{\Tel}\,\Xi$ and by the congruence $\Gamma \dplus [\pi_1(\sigma, t)]_{\Tel}\,\Xi = \Gamma \dplus [ \sigma]_{\Tel}\,\Xi$ holds.
          Then, $\UU_{\Gamma \dplus [\pi_1(\sigma, t)]_{\Tel}\,\Xi} \simeq \UU_{\Gamma \dplus [\sigma]_{\Tel}\,\Xi}$ holds by congruence for $\UU$.
          \LT[noinline]{I think the congruence for $\UU$ is true but not sure how precisely.}
        ...
        \item[$A$ is $\Pi\,B\,C$:]
          ...
          \begin{align*}
            ...
            ={}       & \Pi\,([\sigma \upuparrows \Xi]_{\Ty}\,B)\,([\sigma \upuparrows (\Xi, B)]\,C) && \text{by the induction hypothesises} \\
...
After giving the QIIR definition of a type theory, we have to define how to \emph{eliminate} types $\Ctx$, $\Ty\,\Gamma$, $\Sub\;\Gamma\;\Delta$, and $\Tm\,\Gamma\,A$ altogether to another types (called \emph{motives}) by functions (called \emph{methods}) subject to equations corresponding to point constructors and equality constructors (\cref{subsec:eliminator}).
...
\subsection{Eliminator} \label{subsec:eliminator}
As the same as the QII definition~\cite{Altenkirch2016a}, the motives for the QIIR definition are
\begin{align*}
  \Ctx^A & : \Ctx \to \Set &
  \Sub^A & : \implicit{\Gamma, \Delta}\,\Ctx^A\;\Gamma \to \Ctx^A\;\Delta \to \Sub\;\Gamma\;\Delta \to \Set \\
  \Ty^A  & : \implicit{\Gamma}\,\Ctx^A\;\Gamma \to \Ty\;\Gamma \to \Set & 
  \Tm^A  & : \implicit{\Gamma, A}\; (\Gamma^A : \Ctx^A\;\Gamma) \to \Ty^A\;\Gamma^A\;A \to \Tm\;\Gamma\;A \to \Set
\end{align*}
and each method corresponds to each constructor, e.g., 
\[
  \emptyctx^A : \Ctx^A\;\emptyctx 
  \quad
  \blank ,^A \blank : \implicit{\Gamma, A}\;(\Gamma^A : \Ctx^A\;\Gamma) \to \Ty^A\;\Gamma^A\; A \to \Ctx^A\;(\Gamma, A)
\]
