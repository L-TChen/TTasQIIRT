...
One possibility is to define substitution calculus as a quotient inductive-inductive-\emph{recursive} type whereas type substitution as a simultaneously defined function computes.
...
[ \sigma ]_{\Ty}\;\UU & = \UU
\end{alignat*}
However, this definition is \emph{not} inductive-recursive in the sense of Dybjer and Setzer~\cite{Dybjer2000,Dybjer2003}: in their schema, the codomain of a simultaneously defined function does not refer to any inductive type being defined.
...
\begin{alignat*}{3}
  ...
  []\El & : \implicit{\Gamma, \Delta, \sigma, u} && [ \sigma ]\,(\El\, u) =^{\Tm\;\Gamma\;\UU} \El\,(\transfib{\Tm\;\Gamma}{[]\UU}{[\sigma]{u}})
\end{alignat*}
...
In our QIIR definition, the transported term $\transfib{\Tm\;\Gamma}{[]\UU}{[\sigma]{u}}$ is no longer needed, since the equality constructor $[]\UU$ becomes definitional. 
Therefore, we may replace the equality constructor $[]\El$ by a clause of type substitution:
\begin{alignat}{3}
...
\end{proposition}
\begin{proof}
  In addition to previous cases in the proof of \Cref{prop:local-confluence-1}, consider following terms 
  \[
...
We have seen that the extension of substitution calculus with an (empty) universe breaks the local confluence of type substitution defined in \cref{subsec:SC-QIIRT}, because the newly added function clause interacts with other existing clauses.
...
\begin{alignat}{5}
  ...
  \mathsf{[]\lambda} & : \implicit{\Gamma, \Delta, \sigma, A, B, t} && [\sigma]_{\Tm} (\lambda\,t) && =^{\Tm\,\Gamma}_{[]\Pi} && \lambda\,([\sigma \uparrow A]\,t) \nonumber
\end{alignat}
where $\sigma \uparrow A$ is the \emph{lifting}  of $\sigma$ by a type $A$ defined as $(\pi_1\idS; \sigma, \transfib{\Tm\;\Gamma}{[;]_T}{\pi_2\idS})$. 
...
\begin{proposition}[Coherence]
  ...
  \]
  hold if $\sigma = \tau$ for any substitutions $\sigma$ and $\tau$.
\end{proposition}
\begin{proof}
  These two identities follow from \cref{prop:correctness-2,prop:correctness-3} easily.
\end{proof}

\subsubsection{Coherence of type substitution}
For the coherence of~$[\blank]_{\Ty}\blank$, we have to generalise the desired identity to account for the lifting, since a naive induction does not work: for example, unfolding $[\sigma]_{\Ty}\,(\Pi\;A\;B)$ gives us
\[
...
Observe that its type, generalising the type \eqref{eq:type-of-lifting} of a single lifting $\blank\uparrow\blank$,
\[
\blank\upuparrows \blank : (\sigma : \Sub\;\Gamma\;\Delta) \to (\Xi : \Tel\;\Delta)
\to \Sub\;(\Gamma \dplus ([\sigma]_{\Tel}\,\Xi))\;(\Delta \dplus \Xi)
\]
requires telescope substitution $[\blank]_{\Tel}\blank :  \Sub\;\Gamma\;\Delta \to \Tel\;\Delta \to \Tel\;\Gamma$ defined simultaneously.
Hence, we arrive definitions for telescope substitution and the multiple lifting by a telescope:
\begin{alignat*}{5}
...
Now, we have to prove the coherence property for telescope and type substitution together.
\begin{proposition}
  The identities
  \danger
  \[
  ...
  \]
  hold if $\sigma = \tau$ for any substitutions $\sigma$ and $\tau$, and a telescope $\Xi$.
\end{proposition}
\begin{proof}
...
\end{alignat*}
can be introduced without any transport, which would be $(\idS, \transfib{\Tm\;\Gamma}{[id]_T^{-1}}{t})$ in its QII definition, and which can used to define the ordinal application: $t \mathop{\$} u \defeq [ \left< u \right> ]_{\Tm} (\mathsf{app}\,t)$.
