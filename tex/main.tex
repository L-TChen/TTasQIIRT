...
%
For example, inductive families~\cite{Dybjer1994} can be used to represent the simply typed lambda calculus~\cite{Altenkirch1999}, while some kind of simultaneous definitions such as inductive-recursive~\cite{Dybjer2000} or inductive-inductive~\cite{NordvallForsbergPhD} definitions seem to be required for dependent type theory~\cite{Danielsson2006,Chapman2009}.
%
This is not just a matter of the pure strength of the host theory, but also expressivity and convenience: while it is known how to reduce inductive-inductive definitions to inductive families in principle~\cite{Kaposi2000}, the resulting development would be considerable harder to work with, and most likely would have exhausted the resources of both the proof assistant and its user.

Altenkirch and Kaposi~\cite{Altenkirch2016a} showed that by employing \emph{quotient inductive-inductive types}~\cite{NordvallForsberg2018}, the internal representation of well typed terms could be significantly simplified, since equations such as $\beta$- and $\eta$-equality can be represented by the identity type thanks to quotient constructors.
%
...
%
The basic idea, following a suggestion by Kaposi~\cite{Kaposi2023}, is to replace the inductively defined action of a substitution on a type with a recursive definition, which computes.
%
As a result, more equations will hold strictly, and we can (at least partially) avoid the so-called ``transport hell'' that arises when constructions need to be transported across equality constructors.
%
This is a general phenomenon when working with quotient inductive-inductive types: to make use of equalities in later constructors and when using the eliminator for the type, the equalities need to be transported across.
%
...
%
This seems to be needed in order to again avoid transport hell in the form of equations that do not reduce due to the standard translation of pattern matching to case trees.
%
We emphasise that, following a fine tradition of previous attempts to represent type theory in type theory~\cite{Danielsson2006,Chapman2009,Altenkirch2016a}, we are making use of type-theoretic features that are so far not well understood or foundationally well motivated. Nevertheless, Agda can be convinced to accept our development, using some experimental flags. Indeed, we also see this paper as motivation to study quotient inductive-inductive-recursive types and overlapping patterns further.
...
By design, definitions by pattern matching will be elaborated to a form that corresponds to the \emph{eliminator} either using \emph{first-match semantics}~\cite{Cockx2020a} or an explicit construct for pattern matching.
In the above definition, only the eliminator for $\Ty\;\Delta$ will be considered.
...
It also has a natural setoid model (which should be useful to manifest the difference between recursion and equality constructor) but quotient (inductive-)inductive(-recursive) types for OTT are underdeveloped.}
