...
\begin{alignat*}{5}
  ...
  0              & +' m         && \defeq m  \\
  (\suc\;n)      & +' 0         && \defeq \suc\,n \\
  (\suc\;n)      & +' (\suc\;m) && \defeq \suc\;(n +' m)
\end{alignat*}
\end{minipage}
\vskip\belowdisplayskip\noindent
after elaboration in \Agda: each clause of $\blank +' \blank$ is strict (hence the symbol $\defeq$ is used) while clauses of $\blank+\blank$ are not necessarily strict by definition.
This semantics for function definitions affects how definitions reduce and therefore how identities are proved.

Cockx explores an alternative semantics for pattern matching~\cite{Cockx2014}, where all clauses are strict, even in the presence of \emph{overlapping patterns}.
Under this semantics, each clause of $\blank + \blank$ is strictly equal to its right-hand side, allowing the symmetry of $\blank + \blank$ to be proved commutative directly by induction on $n$ and $m$.
Thus, when a function $f$ is defined with overlapping patterns, we use the notation $f\;\overrightarrow{p_i} \reduce t_i$ to emphasize its reduction behaviour.
...
Cockx shows~\cite[Proposition~2]{Cockx2014} that if a definition of $f$ is
\begin{enumerate}
  \item \emph{structurally recursive}, i.e.\ arguments of recursive calls are structurally smaller, and
...
We call a diagram $\sigma t_2 \Leftarrow f\;\sigma\overrightarrow{p_1} \Rightarrow \sigma t_1$ a \emph{local peak} informally.

\subsection{Overlapping patterns via adding rewrite rules}
...
Then, we can give this QIIR definition of type theory a standard model (\cref{subsec:std-model}) and compare our QIIR definition with its QII counterpart syntactically and semantically (\cref{subsec:equivalence}).
...
\section{Conclusions, and related and future work}
...
\Fredrik{As far as I can tell, ``Type theory in type theory with single substitutions''~\cite{Kaposi2024a} is just a standard QIIT --- so maybe no need to discuss?}
...
years~\cite{NordvallForsberg2017,Kaposi2018,Kovacs2023}, and it seems
