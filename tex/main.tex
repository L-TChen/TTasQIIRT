...
Nevertheless, Agda can be convinced to accept our formal implementation, using some experimental flags.
    ...
    \item Consider the following cases for $[\sigma\upuparrows\Xi]_{\Ty}\,A = [\tau \upuparrows \xi]_{\Ty}\,A$ by induction on $A$.
      \begin{description}
        \item[$A$ is $\UU$:] By definition
          ...
          \LT[noinline]{To fix: Clarify this.}
...
\section{Elimination and models}
\LT{%
\begin{enumerate}
...
With the QIIR definition of a type theory, we show how to \emph{eliminate} $\Ctx$, $\Ty\,\Gamma$, $\Sub\;\Gamma\;\Delta$, and $\Tm\,\Gamma\,A$ to another types (called \emph{motives}) by functions (called \emph{methods}) subject to equations corresponding to point constructors and equality constructors (\cref{subsec:eliminator}).
Then, we can give this QIIR definition of type theory a \emph{standard model} (\cref{subsec:std-model}) and also show that our definition is actually equivalent to its QII counterpart (\cref{subsec:equivalence}).


\subsection{The elimination principle} \label{subsec:eliminator}
To eliminate $\Ctx$, $\Ty\,\Gamma$, $\Sub\;\Gamma\;\Delta$, and $\Tm\,\Gamma\,A$, we have to specify four types
\begin{align*}
  \Ctx^M & : \Ctx \to \Set &
...
\end{align*}
called the \emph{motivates} and functions indexed by corresponding constructors such as 
\begin{alignat*}{3}
  \emptyctx^M     & :                      && \Ctx^M\;\emptyctx \\
  \blank,^M\blank & : \implicit{\Gamma, A} && (\Gamma^M : \Ctx^M\;\Gamma) \to \Ty^M\;\Gamma^M\;A \to \Ctx^M\;(\Gamma, A) \\
  \idS^M          & : \implicit{\Gamma, \Gamma^M}    && \Sub^M\;\Gamma^M\;\Gamma^M\;\idS \\
  \blank;\blank   & : \implicit{\Gamma, \Gamma^M, \Delta, \Delta^M, \Theta, \Theta^M} && \Sub^M\;\Gamma^M\;\Delta^M\;\sigma \to \Sub^M\;\Delta^M\;\Theta^M\;\tau \to \Sub^M\;\Gamma^M\;\Theta^M\;(\sigma;\tau)\\
  [\blank]^M\blank & : \implicit{\Gamma, \Gamma^M, \Delta, \Delta^M, \sigma, A, A^M, t} && (\sigma^M : \Sub^M\;\Gamma^M\;\Delta^M\;\sigma) \to \Tm^M\;\Delta^M\;A^M\;t \\
  &&& \to \Tm^M\;\Gamma^M\;([\sigma^M]_{\Ty}^M\,A^M)([\sigma]\,t)
\end{alignat*}
...
\begin{alignat*}{3}
  ...
  [\blank]_{\Tm}^M\blank & : \implicit{\Gamma, \Gamma^M, \Delta, \Delta^M, \sigma, A, A^M, t} && (\sigma^M : \Sub^M\;\Gamma^M\;\Delta^M\;\sigma) \to \Tm^M\;\Delta^M\;A^M\;t \\
                         &&& \to \Tm^M\;\Gamma^M\;([\sigma^M]_{\Ty}^M\,A^M)([\sigma]_{\Tm}\,t)
\end{alignat*}
called \emph{methods} subject to the corresponding equations \emph{and function clauses}, e.g., 
\begin{alignat*}{5}
...
\end{alignat*}
To define the elimination principle, we cannot avoid the use of transports in identities that would need transports in its QII definition of syntax, such as $[\idS]_{\Tm}^M$, since identities for substitutions (e.g., $[id]_{\Ty}^M$) are still propositional. 
On the other hand, in the elimination principle for the QII definition, the above identity $[\idS]_{\Ty}^M$ already requires $\alert{\transfib{\Ty^M\;\Gamma^M}{[\idS]_{\Ty}}{\color{black}[\idS^M]_{\Ty}^M\,A^M}}$ to coerce its type $\Ty^M\;\Gamma^M\;([\idS]\,A)$ to $\Ty^M\;\Gamma^M\;A$ to match the type of $A^M$.
For brevity, we omit the rest of methods and we refer to the Agda implementation for the complete definition of motives and methods.
\LT[noinline]{Add a link.}

Given the motives and the methods, the elimination principle is defined as functions
\begin{alignat*}{5}
...
\end{alignat*}
by interpreting each constructor using the corresponding method inductively.

In the QII counterpart, $[\blank]\blank$ is a constructor, so $\elim_{\Ty}\;([\sigma]\,A)$ is eliminated to $[\elim_{\Sub}\,\sigma]^M_{\Ty}\,(\elim_{\Ty}\,A)$ definitionally and likewise for the term substitution.
In our QIIR definition, however, these become propositional identities:
\begin{proposition}
  Given the motives and the methods, we have
\begin{alignat*}{5}
  \elim_{\Ty}[] & : \implicit{\Gamma, \Delta, \sigma, A} & \elim_{\Ty}\,([\sigma]_{\Ty}\,A) & = && [\elim_{\Sub}\,\sigma]^M_{\Ty}\,(\elim_{\Ty}\,A) \\
  \elim_{\Tm}[] & : \implicit{\Gamma, \Delta, \sigma, A, t} & \elim_{\Tm}\,([\sigma]_{\Tm}\,t) & =^{\lam{A^M}\Tm^M\;\Gamma^M\;A^M\;t}_{\alert{\elim_{\Ty}[]}} && [\elim_{\Sub}\,\sigma]^M_{\Tm}\,(\elim_{\Tm}\,t)
\end{alignat*}

\end{proposition}
These identities are required to interpret terms involving definitional identities.
For example, consider the case for eliminating an extension $(\sigma, t)$ for $\sigma : \Sub\;\Gamma\;\Delta$ and $t : \Tm\;\Gamma\;([\sigma]_{\Ty}\,A)$:
\[
  \elim_{\Sub}(\sigma, t) = (\elim_{\Sub}\,\sigma) ,^M \; t^M
\]
The eliminated type of $t^M$ is $[\elim_{\Sub}\,\sigma]^M_{\Ty}\,(\elim_{\Ty}\,A)$ instead of the eliminated type $\elim_{\Ty}\;([\sigma]_{\Ty}\,A)$ of $\elim_{\Tm}\;t$, so $t^M$ above have to be the transported term
\[
  \alert{\transfib{\Tm^M\;(\elim_{\Ctx}\,\Gamma)}{\elim_{\Ty}[]}{\color{black}(\elim_{\Tm}\,t)}}
\]
of $\elim_{\Tm}\,t$ along the identity $\elim_{\Ty}[]$. 
\LT{Add some remark regarding the proof}

\subsection{The standard model} \label{subsec:std-model}
\subsection{The equivalence} \label{subsec:equivalence}
