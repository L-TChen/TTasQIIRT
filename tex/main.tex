...
In a proof assistant, functions are typically defined using pattern matching and elaborated into case trees, which correspond to the eliminator, following \emph{first-match} semantics~\cite{Cockx2020a} and employing termination checking.
This elaboration offers the conventional notation, while making computational behaviour opaque.
...
{proposition}{coherenceTel}
  ...
  $\subTel{\Xi}{\sigma} = \subTel{\Xi}{\tau}$ and $\sigma \upuparrows \Xi =^{\Sub\;(\Gamma\dplus \subTel{\Xi}{\sigma})\;(\Delta\dplus\Xi)} \tau \upuparrows \Xi$. \qed
  \danger
\end{restatable}
