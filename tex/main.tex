...
\supplement{The formal development is hosted at the \textsc{GitHub} repository: \url{https://github.com/genetsai95/DTT-QIIRT}.\LT[noinline]{Transfer it to my account?}}%optional, e.g. related research data, source code, ... hosted on a repository like zenodo, figshare, GitHub, ...
...
\begin{abstract}
  Defining type theory in type theory is a long-standing challenge and a benchmark for the strength and expressivity of type theory and a proof assistant.
  Altenkirch and Kaposi used quotient inductive-inductive types to elegantly represent exactly the well-typed terms of type theory, with the price to be paid that terms frequently contain transports along equality constructors.
...
\LT{I am okay with changing it in the paper: This notation is used in the formalisation to avoid using $\comp$ for substitution composition: }
...
To retain the intuitive way of reasoning such as in equation \eqref{eq:pi2-comp-proof} \emph{formally}, we would like to make type substitution rules strict, since the root cause is that type substitution does not compute since it is a constructor, leading to type mismatches occurring with substitution.
...
\]
where we have written $t \reduce t'$ for $t \equiv t'$ to emphasise the reduction behaviour. Thus in this case we have again lost the strict identity $[\sigma]\;\UU \equiv \UU$.
%
...
\begin{remark}\label{re:coherence-proof}
  Note that in the above proof the first four identities hold strictly by \eqref{eq:def-type-subst-1}--\eqref{eq:def-type-subst-4}, while the last three identities follow from \eqref{eq:def-type-subst-5} by induction on $A$.
...
\LT{We may wonder if the last three identities can be made strict.
...
\LT{How about deferring introducing heterogeneous equality until \cref{sec:formal-implementation}?}
\end{proof}
...
\end{align*}
without any transport (cf.\ \cite{Altenkirch2016a}), since \eqref{eq:def-type-subst-8} has become strict. 
...
In the QIIR definition, on the contrary, the type of $\elim_{\Tm}\;t$ is $\elim_{\Ty}\;([\sigma]_{\Ty}\,A)$ is not strictly equal to the RHS of \eqref{eq:elim-subext}, so $\elim_{\Sub}(\sigma, t)$ has to be
\[
  (\elim_{\Sub}\,\sigma) ,_{\Sub}^M \alert{\transfib{\Tm^M\;(\elim_{\Ctx}\,\Gamma)}{\elim_{\Ty}[]}{\color{black}(\elim_{\Tm}\,t)}}
\]
transporting $\elim_{\Tm}\,t$ along the identity $\elim_{\Ty}[]$.
...
\subsection{The \texorpdfstring{$\Set$}{Set}-theoretic model} \label{subsec:std-model}
Our definition has the standard $\Set$-theoretic model, justifying the soundness of our definition, given by the same motives as those for its QII counterpart~\cite[Section~4]{Altenkirch2016a}:
\begin{alignat*}{5}
...
\begin{align*}
  ...
  [\blank]_{\Tm}^M\blank & \defeq \lam{\sigma^M\,t^M} (\sigma^M ; t^M)  \\
...
In the QII counterpart, none of identities hold strictly, because their indices always involve equality constructors.
E.g., the equality constructor $[\idS]_{\Ty} : [\idS]\,A =^{\Ty\,\Gamma} A$ gives rise to
\begin{alignat*}{5}
  [\idS]^M & : [ \idS^M ]^M_{\Ty}\, A^M && =^{\Ty^M\;\Gamma^M}_{\alert{[\idS]}} A^M
\end{alignat*}
The transport along $[\idS]$ is needed, because $[ \idS^M ]^M_{\Ty}\, A^M$ is indexed by $[\idS]\,A$ instead of $A$.
Accordingly, this identity becomes $\lam{\gamma} A^M\,\gamma =_{\alert{[\idS]}}^{\lam{\nameless}\Gamma^M \to \Set} A^M$ in the standard model and needs a step~\cite[Lemma~2.3.5]{UFP2013} of reasoning about the constant transport to show.

In contrast, in our QIIR definition, many identities just hold strictly.
Consider
\begin{alignat*}{5}
  [\idS]^M                   & : [ \idS^M ]^M_{\Ty}\, A^M && = A^M \\
  \idS{\mathord{\uparrow}}^M & : \idS^M \uparrow^M A^M    && =^{\lam{B^M}\Sub^M\;(\Gamma^M, A^M)\;\Delta^M\;(\idS \uparrow A)}_{\alert{[\idS]^M}} \idS^M
\end{alignat*}
where $[\idS]^M$ does not need a transport and is just an strict equality $(\lam{\gamma} A^M\,\gamma) \judgeq A^M$ in the model by the $\eta$-law for $\Pi$-types.
Hence, $\idS{\mathord{\uparrow}}^M$ becomes $(\idS^M \uparrow^M A^M) = \idS^M$, strictly equal to another strict equality $(\lam{\gamma}(\pi_1\gamma, \pi_2\gamma)) \judgeq \lam{\gamma}\gamma$ by the $\eta$-law for $\Sigma$-types.

The additional rules we have used to enable further reduction are derivable already, and they can be justified by the same standard model with simpler proofs.
Now is nature to ask: what is the theoretical difference between the QIIR and QIIR definitions?
