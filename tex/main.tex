...
\newcommand{\Fredrik}[2][]{\todo[inline,author={Gene},caption={},#1]{#2}}
...
To retain the intuitive way of reasoning such as \eqref{eq:pi2-comp-proof} \emph{formally}, we would like to make type substitution rules definitional, since the root cause is that type substitution as a constructor does not compute so that type mismatches occur after substitution.
...
In this case, only the eliminator for $\Ty\;\Delta$ will be considered in the above definition.
...
Then, the equality constructor $[]\El$ and the rule $\mathsf{concat}$ are accordingly changed to 
\begin{align}
...
To prove the coherence property for the multiple lifting, we would need the properties $(\idS\;\{\Gamma\}) \upuparrows \Xi = \idS\;\{\Gamma \dplus  \Xi\}$ and $(\sigma; \tau) \upuparrows \Xi = \sigma; \tau^+$, similar to \cref{lem:lifting}, but which cannot be typed: the type of $\idS \upuparrows \Xi$ is $\Sub\;(\Gamma \dplus [\idS]_{\Tel}\,\Xi)\;(\Gamma \dplus \Xi)$ while the type of $\idS$ on the RHS is $\Sub\;(\Gamma \dplus \Xi)\;(\Gamma \dplus \Xi)$; similarly for the substitution composition.

Instead of resorting to use \transp, we also turn telescope substitution $[\blank]_{\Tel}\blank$ and the multiple lifting into definitions by overlapping patterns by adding following clauses:
...
\begin{proof}
  ...
  \begin{align*}
    ...
    [\sigma; (\tau, t)]_{\Ty} \;A    & = [(\sigma;\tau), [\sigma]\, t]_{\Ty}\;A,
                                     ...
                                     & [\sigma]_{\Ty} \;A               & = [\pi_1\sigma, \pi_2\sigma]_{\Ty}\;A
  \end{align*}
  Identities in the firs row are straightforward (not definitional, though).
