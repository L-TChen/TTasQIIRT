...
\begin{example}\label{ex:pi2-comp}
  ...
  \]
  apparently holds and can checked by a back-of-the-envelope calculation
...
{proposition}{coherenceTel}
  ...
  \begin{align*}
    \subTel{\Xi}{\sigma} & = \subTel{\Xi}{\tau}
                         && \sigma \upuparrows \Xi =^{\lam{\gamma}\Sub\;(\Gamma\dplus \subTel{\Xi}{\gamma})\;(\Delta\dplus\Xi)}_{\alert{\mathsf{c}}} \tau \upuparrows \Xi \\
    \subTy{A}{\sigma \upuparrows \Xi} & =^{\lam{\gamma}\Ty\;(\Gamma \dplus \subTel{\Xi}{\gamma})}_{\alert{\mathsf{c}}} \subTy{A}{\tau \upuparrows \Xi}.
  \end{align*}
\end{restatable}
...
\begin{alignat*}{5}
  [\idS]_{\Ty}^M & : \implicit{\Gamma, \Gamma^M, A, A^M}\; && \subTyM{A^M}{\idS^M} && =^{\Ty^M\,\Gamma^M}\;&& A^M %\\
...
\begin{remark}
  ...
  \[
    [\idS]_{\Tm}^M : \implicit{\Gamma^M, t, t^M} \;\subTmM{t^M}{\idS^M} =^{\lam{A^M}\Tm^M\;\Gamma^M\;A^M\;t}_{\alert{[\idS]^M_{\Ty}}} \; t^M
  \]
...
\begin{alignat*}{5}
  ...
  []\mathsf{refl} & : \implicit{\Gamma, a, t}\;&& \sub{(\mathsf{refl}\;t)}{\sigma} =^{\Tm\;\Gamma\;(\sub{(\mathsf{Id}\,a\,t\,t)}{\sigma})} \mathsf{refl}(\sub{t}{\sigma}) \\
...
\begin{alignat*}{5}
  ...
  [\pi_1;]^M_{\Ty}    & : \subTyM{A^M}{\pi_1^M\,(\sigma^M ;^M \tau^M)}     && = \left(\subTyM{A^M}{\pi_1^M\,\tau^M}\right) [\sigma^M]^M_{\Ty} \\
...
\begin{alignat*}{5}
  ...
  \mathsf{concat}^M         & : \sigma^M ;^M (\tau^M ,_{\Sub}^M t^M) && =^{\Sub^M\,\Gamma^M\,\Delta^M}_{\alert{\mathsf{concat}}} && (\sigma^M ;^M \tau^M) ,_{\Sub}^M \alert{\transfib{}{([;]^M)^{-1}}{\color{black} \subM{t^M}{\sigma^M}}} \\
...
\begin{alignat*}{5}
  []\Pi^M         & : \subTyM{(\Pi^M\;A^M\;B^M)}{\sigma^M}  && =^{\Ty^M\,\Gamma^M\,\left(\subTy{(\Pi\,A\,B)}{\sigma}\right)} {} && \Pi^M\;(\subTyM{A^M}{\sigma^M})\;(\subTyM{B^M}{\sigma^M}) \\
