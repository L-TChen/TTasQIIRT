...
\begin{proof}
  ...
  \]
  and show that each applicable clause does result in the same term $\UU$.
  ...
  It is easy to see that remaining local peaks reduce to the same term $\UU$.
...
In our QIIR definition, the transported term $\transfib{\Tm\;\Gamma}{[]\UU}{[\sigma]{u}}$ is no longer needed, since $[]\UU$ is definitional. 
Therefore, we may replace the equality constructor $[]\El$ by a clause for type substitution:
\begin{alignat}{3}
  [ \sigma ]_{\Ty}\,(\El\, u) & \reduce \El\,([\sigma] {u}) \label{eq:def-type-subst-6}
\end{alignat}
Yet, this naive clause breaks the local confluence!
...
To repair the local confluence, we add a simultaneous recursive function $[\blank]_{\Tm}\blank$ for term substitution apart from the explicit term substitution $[\blank]\blank$ as follows
\begin{alignat*}{3}
...
\end{proposition}
\begin{proof}
  Again, in addition to those in the proof of \Cref{prop:local-confluence-1}, consider following terms 
  \[
...
\begin{proof}
  ...
  However, all cases follow easily from \Cref{prop:correctness}.
