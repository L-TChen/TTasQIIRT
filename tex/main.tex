...
The recursor for $(\blank)^{\succ}$ is given straightforwardly: each method $c^M_{\IR}$ is given by the corresponding constructor $c_{\I}$ in the QIIT counterpart as well as `smart constructors'.
...
\Fredrik{I would be surprised if this was equivalent to UIP. I would suggest to delete the remark.}}
since the method $\mathsf{concat}^M_{\I}$ has a transport along $[;]_{\Ty}^M$ but $\mathsf{concat}_{\IR}$ does not.

Going back and forth from one definition to the other gives us the isomorphism:
\begin{theorem}
  $\Ctx_{\IR}$ is isomorphic to $\Ctx_{\I}$.
  In particular, $(\blank)^{\prec} : \Ctx_{\I} \to \Ctx_{\IR}$ and $(\blank)^{\succ} : \Ctx_{\IR} \to \Ctx{\I}$ are inverses to each other. 
\end{theorem}
