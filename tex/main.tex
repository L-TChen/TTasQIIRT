...
\begin{alignat*}{3}
  \Jnil & : \JList{X} \\
  \Jsing{-} & : X \to \JList{X} \\
  \Jconcat{-}{-} & : \JList{X} \to \JList{X} \to \JList{X} \\
...
\Fredrik{Give an actual example of QIIT (or even QIRT)??}
...
Then, we can give this QIIR definition of type theory a \emph{standard model} (\cref{subsec:std-model}) and also show that our definition is actually equivalent to its QII counterpart syntactically and semantically (\cref{subsec:equivalence}).
...
Given an eliminator, the computation is given to the following function clauses
\begin{alignat*}{5}
  ...
  \elim_{\Sub}\,\emptysub   & \reduce \emptysub^M_{\Sub} &&: \Sub^M\;\Gamma^M\;\emptyctx^M_{\Ctx}\;\emptyctx \\
                            & \vdots
\end{alignat*}
by interpreting each constructor using the corresponding method inductively, similar to its QII counterpart.
Here, the difference is that type substitution $[\blank]\blank$ was a constructor and $[\sigma]\,A$ was eliminated to $[\elim_{\Sub}\,\sigma]^M_{\Ty}\,(\elim_{\Ty}\,A)$; similarly for term substitution.
In contrast, substitution operations $[\blank]_{\Ty}\blank$ and $[\blank]_{\Tm}$ are functions in our QIIR definition that cannot be eliminated inductively, but they become propositional identities:
...
These identities are required to interpret constructors using definitional identities.
E.g., consider eliminating an extension $(\sigma, t)$ for $\sigma : \Sub\;\Gamma\;\Delta$ and $t : \Tm\;\Gamma\;([\sigma]_{\Ty}\,A)$, i.e.\ 
\[
...
In the QIIR definition, on the contrary, the type of $\elim_{\Tm}\;t$ is $\elim_{\Ty}\;([\sigma]_{\Ty}\,A)$ which is not definitionally equal to the RHS of \eqref{eq:elim-subext}, so $\elim_{\Sub}(\sigma, t)$ is in fact
\[
  (\elim_{\Sub}\,\sigma) ,_{\Sub}^M \alert{\transfib{\Tm^M\;(\elim_{\Ctx}\,\Gamma)}{\elim_{\Ty}[]}{\color{black}(\elim_{\Tm}\,t)}}
\]
requiring a transport of $\elim_{\Tm}\,t$ along the identity $\elim_{\Ty}[]$. 
...
Our QIIR definition of type theory has a standard model by giving the following motives and methods.
